...
<!--    
    ...
    <xsl:template match="teiHeader"/>
    <xsl:template match="sourceDoc"/>
    <xsl:template match="orig"/> <!-\- Skip original form -\->
    
    <!-\- Handle <choice> by selecting only <reg> -\->
    <xsl:template match="choice">
        <xsl:apply-templates select="reg"/>
    </xsl:template>
    
    <!-\- Add line break for every <lb> -\->
    <xsl:template match="lb">
        <xsl:text>\\
</xsl:text>
    </xsl:template>
    
    <!-\- Match <reg>: escape content and add line break -\->
...
\section*{</xsl:text><xsl:apply-templates/><xsl:text>}</xsl:text>
    ...
    <!-\- Safely extract only text (used for LaTeX content) -\->
